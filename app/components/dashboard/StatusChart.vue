  ...
  <div class="space-y-3">
    ...
    <div class="space-y-2">
      <div class="flex items-center gap-2">
        ...
        <div class="h-2 flex-1 overflow-hidden rounded bg-secondary">
          <div
            class="h-full rounded bg-[var(--color-status-open)] transition-all"
            :style="{ width: `${openPercent}%` }"
          />
        </div>
        <span class="w-8 text-right text-xs">{{ open }}</span>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="h-2 flex-1 overflow-hidden rounded bg-secondary">
          <div
            class="h-full rounded bg-[var(--color-status-closed)] transition-all"
            :style="{ width: `${closedPercent}%` }"
          />
        </div>
        <span class="w-8 text-right text-xs">{{ closed }}</span>
      </div>
    </div>
  </div>
</template>
